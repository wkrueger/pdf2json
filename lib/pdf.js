...
let PDFJSClass = (function () {
  ...
  cls.prototype.loadPages = function () {
    ...
    if (!this.options.pages) {
      ...
      pagesToRender = this.options.pages
    }
    let pagePromises = this.options.pages.map((p) => this.pdfDocument.getPage(p))

    let waitAllPages = PDFJS.Promise.all(pagePromises)

    nodeUtil.p2jinfo("PDF loaded. pagesCount = " + pagesCount)

    return waitAllPages.then(
      (promisedPages) => this.parsePage(promisedPages, 0, 1.5),
      (error) => this.raiseErrorEvent("pagesPromise error: " + error)
    )
  }

  cls.prototype.parsePage = function (promisedPages, index, scale) {
    nodeUtil.p2jinfo("start to parse page:" + (index + 1))

    let pdfPage = promisedPages[index]
    let pageParser = new PDFPageParser(pdfPage, index, scale, this.ptiParser)

    function continueOnNextPage() {
      nodeUtil.p2jinfo("complete parsing page:" + (index + 1))
      if (index >= promisedPages.length - 1) {
        ...
        process.nextTick(() => this.parsePage(promisedPages, ++index, scale))
      }
    }
        ...
        if (this.needRawText) {
          pdfPage.getTextContent().then(
            (textContent) => {
              ...
              nodeUtil.p2jinfo("complete parsing raw text content:" + (index + 1))
